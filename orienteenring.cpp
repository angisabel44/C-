...
public:
	...
	int calcularDistancia(std::list<coordenada>::iterator it, std::vector<coordenada>::iterator jt);
	...
	void guardarSolucionConstructivo();
	void guardarSolucionBusquedaLocal();
	void impresion();
...
void orienteering::funDisInicial()
{
	...
	distancia = round(sqrt(pow(deltax, 2) + pow(deltay, 2)));
}
...
void orienteering::agregarElemento(std::list<coordenada>::iterator it, std::vector<coordenada>::iterator lt)
{
	...
	int a = round(sqrt(pow(jt->x - lt->x, 2) + pow(jt->y - lt->y, 2)));
	int b = round(sqrt(pow(kt->x - lt->x, 2) + pow(kt->y - lt->y, 2)));
	int c = round(sqrt(pow(jt->x - kt->x, 2) + pow(jt->y - kt->y, 2)));
...
void orienteering::borrarElemento(std::vector<coordenada>::iterator it)
{
	...
	int a = round(sqrt(pow(jt->x - it->x, 2) + pow(jt->y - it->y, 2)));
	int b = round(sqrt(pow(kt->x - it->x, 2) + pow(kt->y - it->y, 2)));
	int c = round(sqrt(pow(jt->x - kt->x, 2) + pow(jt->y - kt->y, 2)));
...
void orienteering::guardarSolucionConstructivo()
{
	string salida = "resultados";
	ofstream archivo(salida.c_str(), ios::app);

	archivo << instancia << " " << funcionObjetivo << " ";

	archivo.close();
}

void orienteering::guardarSolucionBusquedaLocal()
{
	string salida = "resultados";
	ofstream archivo(salida.c_str(), ios::app);
	...
	archivo << mejorObjetivo << " " << ejecucion << " " << iteraciones - 10 << endl;

	archivo.close();
}

int orienteering::calcularDistancia(std::list<coordenada>::iterator it, std::vector<coordenada>::iterator jt)
{
	std::vector<coordenada>::iterator i = jt - 1;

	int a = round(sqrt(pow(i->x - it->x, 2) + pow(i->y - it->y, 2)));
	int b = round(sqrt(pow(jt->x - it->x, 2) + pow(jt->y - it->y, 2)));
	int c = round(sqrt(pow(i->x - jt->x, 2) + pow(i->y - jt->y, 2)));
...
std::vector<coordenada>::iterator orienteering::mejorPosicion(list<coordenada>::iterator it)
{
	...
	int dist = calcularDistancia(it, jt);
...
std::vector<coordenada>::iterator orienteering::quienQuito()
{
	...
	float a = round(sqrt(pow(jt->x - it->x, 2) + pow(jt->y - it->y, 2)));
	float b = round(sqrt(pow(lt->x - it->x, 2) + pow(lt->y - it->y, 2)));
	...
	for (std::vector<coordenada>::iterator i = solucion.begin() + 1; i != solucion.end() - 1; ++i)
	{
		...
		a = round(sqrt(pow(jt->x - i->x, 2) + pow(jt->y - i->y, 2)));
		b = round(sqrt(pow(lt->x - i->x, 2) + pow(lt->y - i->y, 2)));
